...
def get_kb(cases_tags, memes_kb):
    cases_know = {}
    for m in cases_tags:
        if m.replace(".png", "") in list(memes_kb.keys()):
            if pd.isna(memes_kb[m.replace(".png", "")]) != True:
                cases_know[m] = memes_kb[m.replace(".png", "")]
            else:
                cases_know[m] = ""
        else:
            cases_know[m] = ""
...
def kb_inference(train_mode="manual", test_mode="manual"):
    dev_path = "memegraphs/data/Validation_meme_dataset.csv"
    test_path = "memegraphs/data/Testing_meme_dataset.csv"

    # Read data
    # memes_know_df = pd.read_csv("multi_off_sg_kb_new.csv")
    # memes_know_df[["ent_texts", "obj_texts"]] = memes_know_df[["ent_texts", "obj_texts"]].fillna(value="")
    # memes_know = {}
    # for index, row in memes_know_df.iterrows():
    #     if row.ent_texts == "" and row.obj_texts == "":
    #         memes_know[row.filename] = ""
    #     elif row.ent_texts == "":
    #         memes_know[row.filename] = row.obj_texts + "."
    #     else:
    #         memes_know[row.filename] = row.ent_texts + ". " + row.obj_texts + "."

    if train_mode != test_mode:
        memes_know_df = pd.read_csv("memegraphs/data/multi_off_entities_automatic.csv")
        memes_know_manual_df = pd.read_csv("memegraphs/data/memegraphs_sg_kb.csv")
        memes_know_manual = dict(zip(memes_know_manual_df["filename"], memes_know_manual_df["knowledge_base"]))
    elif train_mode == "manual":
        memes_know_df = pd.read_csv("memegraphs/data/memegraphs_sg_kb.csv")
    else:
        memes_know_df = pd.read_csv("memegraphs/data/multi_off_entities_automatic.csv")

    memes_know = dict(zip(memes_know_df["filename"], memes_know_df["knowledge_base"]))

    # memes_know_manual_df = pd.read_csv("memegraphs_sg_kb.csv")
    # memes_know_manual = dict(zip(memes_know_manual_df["filename"], memes_know_manual_df["knowledge_base"]))
    ...
    test_cases_know = get_kb(test_cases_tags, memes_know)
    dev_cases_know = get_kb(dev_cases_tags, memes_know)
    ...
    if train_mode != test_mode:
        train_cases_know = get_kb(train_cases_texts, memes_know_manual)
    else:
        train_cases_know = get_kb(train_cases_texts, memes_know)
