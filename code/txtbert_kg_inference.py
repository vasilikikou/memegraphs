...
def get_kg(cases_tags, memes_kg):
    cases_know = {}
    for m in cases_tags:
        if m.replace(".png", "") in list(memes_kg.keys()):
            if pd.isna(memes_kg[m.replace(".png", "")]) != True:
                cases_know[m] = memes_kg[m.replace(".png", "")]
            else:
                cases_know[m] = ""
        else:
            cases_know[m] = ""
...
def kg_inference(train_mode="manual", test_mode="manual"):
    ...
    if train_mode != test_mode:
        memes_kg_df = pd.read_csv("memegraphs/data/multi_off_entities_automatic.csv")
        memes_kg_manual_df = pd.read_csv("memegraphs/data/memegraphs_sg_kb.csv")
        memes_know_manual_b = dict(zip(memes_kg_manual_df["filename"], memes_kg_manual_df["knowledge_base"]))
        ...
        memes_kg_df = pd.read_csv("memegraphs/data/memegraphs_sg_kb.csv")
        memes_sg_df = pd.read_csv("memegraphs/data/multi_off_sc_new.csv")
    else:
        memes_kg_df = pd.read_csv("memegraphs/data/multi_off_entities_automatic.csv")
        memes_sg_df = pd.read_csv("memegraphs/data/multi_off_sc_automatic.csv")

    memes_know_b = dict(zip(memes_kg_df["filename"], memes_kg_df["knowledge_base"]))
    memes_sg = dict(zip(memes_sg_df["filename"], memes_sg_df["scene_graph"]))

    # Create texts for knowledge graphs
    memes_know = {}
    for m in memes_know_b:
        if m in list(memes_sg.keys()):
            if not pd.isna(memes_know_b[m]) and not pd.isna(memes_sg[m]):
                memes_know[m] = memes_know_b[m] + ". " + memes_sg[m]
            elif pd.isna(memes_know_b[m]) and pd.isna(memes_sg[m]):
                memes_know[m] = ""
            elif pd.isna(memes_know_b[m]):
                memes_know[m] = memes_sg[m]
            else:
                memes_know[m] = memes_know_b[m]
        else:
            if not pd.isna(memes_know_b[m]):
                memes_know[m] = memes_know_b[m]
            else:
                memes_know[m] = ""
    ...
    if train_mode != test_mode:
        memes_know_manual = {}
        for m in memes_know_manual_b:
            if m in list(memes_sg_manual.keys()):
                if not pd.isna(memes_know_manual_b[m]) and not pd.isna(memes_sg_manual[m]):
                    memes_know_manual[m] = memes_know_manual_b[m] + ". " + memes_sg_manual[m]
                elif pd.isna(memes_know_manual_b[m]) and pd.isna(memes_sg_manual[m]):
                    memes_know_manual[m] = ""
                elif pd.isna(memes_know_manual_b[m]):
                    memes_know_manual[m] = memes_sg_manual[m]
                else:
                    memes_know_manual[m] = memes_know_manual_b[m]
            else:
                if not pd.isna(memes_know_manual_b[m]):
                    memes_know_manual[m] = memes_know_manual_b[m]
                else:
                    memes_know_manual[m] = ""
    ...
    test_cases_know = get_kg(test_cases_tags, memes_know)
    ...
    dev_cases_know = get_kg(dev_cases_tags, memes_know)
    ...
    if train_mode != test_mode:
        train_cases_know = get_kg(train_cases_texts, memes_know_manual)
    else:
        train_cases_know = get_kg(train_cases_texts, memes_know)
    ...
    # Calculate max len
    lens = [len(bert_tokenizer.encode(train_cases_texts[c], train_cases_know[c])) for c in list(train_cases_texts.keys())]
    ...
    x_test = bert_tokenizer.__call__(
            list(test_cases_texts.values()), list(test_cases_know.values()),
            max_length=max_len,
            padding=True,
            truncation=True,
    ...
    x_dev = bert_tokenizer.__call__(
            list(dev_cases_texts.values()), list(dev_cases_know.values()),
            max_length=max_len,
            padding=True,
            truncation=True,
            return_token_type_ids = True,
    ...
    for k in range(num_runs):
        model_name = "txt_bert_kg_" + train_mode + str(k + 1)
